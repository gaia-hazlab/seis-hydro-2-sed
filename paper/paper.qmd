...
::: {.callout-note}
## PlanetScope pre/post imagery (in preparation)
The avulsion interpretation of the cross-AR baseline drift makes a falsifiable
prediction: the active thread(s) nearest CC.PR01/PR02/PR03 should have shifted
*toward* the stations between the pre-flood and post-flood states. High-cadence
PlanetScope optical imagery bracketing the December-2025 floods (pre- vs post-event
channel maps) will be added to test this directly — quantifying wetted-channel
position relative to each sensor and converting the seismic baseline drift into a
measured channel-migration distance.
:::
...
- **Bedload band unsampled for this event (a data-archival gap, not a permanent one).**
  For December 2025 only the 50-sps (BH) CC waveforms are archived (25 Hz Nyquist),
  so the 30–80 Hz band is absent and the 1–15 Hz signal is turbulence-dominated —
  bedload here is an upper bound, not a measurement (@fig-spec). The 500-sps (CH)
  upgrade came online in 2026, and its high-rate data are not yet retrievable through
  FDSN; obtaining them from CVO/PNSN is the route to a direct 30–80 Hz test. For the
  2025 event the best available probe is the 100-sps UW broadband stations (RER, LON)
  at the 30–50 Hz lower bedload edge.
